...
router.post('/', requireAuth, (req, res) => {
  try {
    const { receiver_id, itinerary_id, date_label, time_label, event_date, message } = req.body;
    const result = db.prepare(
      `INSERT INTO invites (sender_id, receiver_id, itinerary_id, date_label, time_label, event_date, message) VALUES (?, ?, ?, ?, ?, ?, ?)`
    ).run(req.session.userId, receiver_id, itinerary_id, date_label || '', time_label || '', event_date || '', message || '');
